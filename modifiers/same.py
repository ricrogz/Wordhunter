...
class ModifierGenerator(object):
    ...
    def generate(self, word, round_name, involved_letters, difficulty):
        if round_name == "subanag":
            ...
            twodupes = [[r, s]]
        elif round_name in ["blockbeginend", "blockend", "blockmiddle", "blockstart", "exclude", "onlyhas", "ordered",
                            "boggle"]:
            # Find all letters that occur at least twice within the word,
            # logging only their first and second appearances.
            twodupes = [y[0:2] for y in [[i for i in range(len(word)) if word[i] == x] for x in set(word)] if
                        len(y) > 1]
        else:
            twodupes = None
        if twodupes:
            # Purely random choice often forces players to seek extraordinarily long words,
            # so we minimize the forced word length.
            dupe = min(twodupes, key=max)
            regex = re.compile("".join(["^", "." * dupe[0], "(.)", "." * (dupe[1] - dupe[0] - 1), r"\1"]))
            s = STR_ANNOUNCE_MOD.format(embolden(ORDINALS[dupe[0]]), embolden(ORDINALS[dupe[1]]))
        else:
            regex = None
            s = ""
        return regex, s
