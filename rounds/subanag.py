...
class RoundGenerator(object):
    ...
    def generate(self, word, words, difficulty):
        ...
        letters = discrete_sample(constants.VOWEL_FREQS, num_vowels) + discrete_sample(constants.CONS_FREQS,
                                                                                       num_letters - num_vowels)
        random.shuffle(letters)
        s = STR_ANNOUNCE.format(embolden("".join(letters)))
        involved_letters = letters
        return self.subanag_regex(letters).match, s, involved_letters
