...
LETTERFREQS = ["E", "T", "A", "I", "O", "N", "S", "H", "R", "L", "D", "C", "U", "M", "W", "F", "G", "Y", "P", "B", "V",
               "K", "J", "X", "Q", "Z"]
...
class RoundGenerator(object):
    ...
    def generate(self, word, words, difficulty):
        ...
        letters = sorted(set(discrete_sample(constants.VOWEL_FREQS, num_vowels)
        ...
        regex = "^[^" + "".join(letters) + "]*$"
        s = STR_ANNOUNCE.format(listtostr(map(embolden, letters), conj="and"))
        involved_letters = letters
        return re.compile(regex).match, s, involved_letters
