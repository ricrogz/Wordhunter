...
SYNS_DESTROYS = [
    "smashes", "destroys", "wipes the floor with", "trounces", "crushes",
    "annihilates", "decimates", "obliterates"
]
SYNS_DESTROYED = [
    "smashed", "destroyed", "annihilated", "decimated", "obliterated"
]
SYNS_COMPLETELY = ["completely", "utterly", "totally", "absolutely"]

SYNS_FALL_TO = [
    "drop{} to", "fall{} to", "slip{} to", "plummet{} to",
    "get{} knocked down to"
]
...
SYNS_FIRST_POS = [
    "In first place it's {} with {} points!",
    "{} is the champion with {} points!", "{} came out on top with {} points!",
    "{} takes gold with {} points!"
]
SYNS_JOINT_FIRST_POS = [
    "In first place it's {} with {} points apiece!",
    "{} are the champions with {} points apiece!",
    "{} came out on top with {} points each!",
    "{} take gold with {} points each!"
]
SYNS_SECOND_POS = [
    "{} came in second with {}.", "{} takes silver with {}.",
    "{}'s {} points were only enough for second."
]
SYNS_JOINT_SECOND_POS = [
    "{} both came in second with {}.", "{} take silver with {} points apiece.",
    "{}'s {} points were only enough for second."
]
SYNS_THIRD_POS = [
    "{} is bringing up the rear with {}.", "{} makes do with bronze and {}.",
    "{} managed a mere third with {}."
]
SYNS_JOINT_THIRD_POS = [
    "{} are bringing up the rear with {} points each.",
    "{} make do with bronze and {}.",
...
def STRF_GOOD_WORD():
    return " ".join(
        [random.choice(SYNS_YES), "{} fits perfectly for {} points!"])


def STRF_BEAT_SELF():
    return " ".join(
        [random.choice(SYNS_YES), "{} fits even better for {} points!"])


def STRF_BEAT_OTHER():
    return " ".join([
        random.choice(SYNS_YES), "{} ({} points)",
        random.choice(SYNS_DESTROYS), "{}'s {} ({} points)!"
    ])


def STRF_NO_BEAT_OTHER():
    return " ".join([
        random.choice(SYNS_NO), "{} ({} points) doesn't", "beat",
        "{} {} ({} points)!"
    ])
...
def STRF_BEAT_STREAK():
    return " ".join([
        "{}",
        random.choice(SYNS_COMPLETELY),
        random.choice(SYNS_DESTROYED), "{}'s winning streak of {}!"
    ])
...
def STRF_POS(pos, joint=False):
    ret = "joint " if joint else "" + ORDINALS[pos]
    # ret += ("" if random.randint(0,1) == 0 else " place")
    return ret


def STRF_HAS_POS(pos, joint=False):
    return " ".join(
        ["{}", "are" if joint else "is",
         STRF_POS(pos, joint), "with {}!"])


def STRF_STRENGTHENS_POS(pos):
    return " ".join([
        "{}",
        "strengthens their {} place position!".format(STRF_POS(pos,
                                                               joint=False))
    ])
