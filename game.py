...
class WHGame(object):
        ...
        if nick:
            if self.streak["nick"] == nick:
                ...
                if cfg.STREAK_BONUS > 0:
                    ...
                    msg = chatter.STR_ON_STREAK.format(self.streak["nick"], self.streak["num"])
                self.output(msg)
                return bonus
            else:
                if self.streak["num"] > 1:
                    self.output(chatter.STRF_BEAT_STREAK().format(nick, self.streak["nick"], self.streak["num"]))
    ...
    def merge_data(self, lst, pos):  # When merging rank changes, use pos=1 for origin, 2 for destination
        merged, used = [], []
    ...
    def get_rank_changes(self, old_merged, new_merged):
        ...
        changes = [(x, self.merged_pos(x, old_merged) if self.oldscores[x] > 0 else float('inf'),
                    self.merged_pos(x, new_merged) if self.newscores[x] > 0 else float('inf')) for x in
                   self.newscores.keys()]
    ...
    def get_move_text(self, direction, movers, destposnicks, destpos):
        verb = random.choice(chatter.SYNS_FALL_TO if direction == 0 else chatter.SYNS_RISE_TO)
            ...
            if len(rank_changes) == 1 and rank_changes[0][1] == float("inf"):  # First score of the game
                ...
                for x in rank_changes:
                    ...
                    if nick not in used:
                        ...
                        if x[1] != x[2] or len(
                                oldconicks) > 1:  # Movers and shakers (including those moving out of joint positions)
                            direction = int(x[2] <= x[1])  # 1 for upward movement, 0 for downward
                            # The first element of these message tuples is the priority (lower=higher priority).
                            messages.append((0, x[2], self.get_move_text(direction, movers, newconicks, x[2])))
                            used += newconicks
                        else:  # Sitting pretty
                            if len(movers) == 1 and movers[0] == winner:
                                messages.append((1, x[2], self.get_strengthen_text(winner, x[1])))
                            else:
                                messages.append((2, x[2], self.get_stay_text(movers, x[1])))
    ...
    def submit_word(self, word, nick):
        if nick not in self.newscores:
            self.newscores[nick] = 0  # Mark the submitter as having submitted a word (for scoring purposes).
        ...
        if re.match('NESS(?:ES)?$', word):
            ...
            if score > self.winningword_score:
                if self.winningword_score == 0:
                    ...
                    msg = chatter.STRF_BEAT_OTHER().format(word, score, self.winningnick, self.winningword,
                ...
                if word == self.winningword:
                    ...
                    msg = chatter.STRF_NO_BEAT_OTHER().format(word, score, poss, self.winningword,
                                                              self.winningword_score) + " " + self.time_warning()
            self.output(msg)
            ...
            if newindices - self.hint_indices:
                ...
                partial = "".join(
                    [self.answord[i] if i in sorted(allindices) else "?" for i in range(len(self.answord))])
        ...
        if len(self.possible_words) == 0:
            ...
            if mod_regex:
                self.possible_scored_words = dict(
                    (k, v) for k, v in self.scored_words.items() if (validmatch(k) and mod_regex.match(k)))
            else:
                self.possible_scored_words = dict((k, v) for k, v in self.scored_words.items() if validmatch(k))
            self.sorted_possible_scored_words = sorted(self.possible_scored_words.items(), key=itemgetter(1),
                                                       reverse=True)
    ...
    def announce_puzzle(self, reannounce=False):
        ...
        if self.num_rounds == 0:
            round_announce = chatter.STR_ROUND_NUM_UNLIMITED.format(self.round_num)
        elif self.round_num == self.num_rounds:
            round_announce = chatter.STR_ROUND_FINAL
        else:
            round_announce = chatter.STR_ROUND_NUM.format(self.round_num, self.num_rounds)
